...
- **`BudgetPage.tsx` (1396 lines)**: extract the per-page layout state (`collapsedMonths`, `expandedTransferAnchors`, `extraHistory`, `extraFuture`, `viewerOpen`, `conflictsOpen`, `metadataOpen`) into a `useBudgetLayoutState(key)` hook in `components/budget/hooks/`. Today the state is in-memory; the hook is the right shape to grow sessionStorage hydration into later.
...
- **Touch-gesture base hook**: `usePullToRefresh` (204), `useRowSwipe` (85), `useSheetSwipe` (184) duplicate touch-tracking refs and axis-discrimination logic (`Math.abs(dx) > Math.abs(dy)` etc.). Extract `useTouchGesture(orientation, thresholdPx, onGesture)` so each gesture only declares its own state machine. New abstraction — design pass first. Pull-to-refresh's extra damping layer (`RESISTANCE`, `MAX_PULL`) should stay opt-in via parameters so row / sheet swipes don't pay for it. (The shared `hooks/dom-queries.ts` helpers already split out, May 2026.)
