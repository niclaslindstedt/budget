...
- **`SettingsModal.tsx` (812 lines, at `SettingsModal/SettingsModal.tsx`)**: the `tabs/` folder is already in place but the root modal owns tab routing, footer buttons, and dirty-state tracking. Move tab definitions to a registry array `{ id, label, icon, Component }` so adding a new tab is one entry, not a touch on the root. The `tabIds` memo at line 316 already encodes the devMode filter; the registry codifies it.
...
- **`validate/` createEnumValidator polish**: the domain split has landed (8 submodules under `src/data/validate/`); the remaining polish is to hoist the recurring "declare Set from a constant → check membership → cast" pattern (~11 Sets, ~15 `.has()` call sites) into `createEnumValidator<T>(values, key)` in `validate/helpers.ts`. The companion `softValidate<T>(value, default, schema)` wrapper is less justified — only ~3 actual try-catch fallback sites exist in the tree today, not 10+. Polish, not necessity.
