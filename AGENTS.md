...
- **i18n namespace split**: `i18n/locales/en/settings.ts` (387 lines) and `achievements.ts` (402 lines) are flat objects covering many tabs / tiers. Split each into per-tab / per-tier files inside a sibling folder and compose them in the namespace's `index.ts`. Mirror in `sv/`. TypeScript intersection types compose the per-tab `<Ns>Catalog` types cleanly at zero runtime cost.
