...
Notes left from a 2026-05 readability sweep, re-verified against the
current tree. Each line is a discrete follow-up — none are urgent, all
improve discoverability or make a specific feature easier to extend.
Pick one at a time; do not bundle. Verify the line counts and "still
pending" status when you pick one up — the list has gone stale before.

Recently completed (2026-05): the `constants.ts` taxonomy / theme
split (now `src/data/presets.ts` + `src/data/themes.ts`), the
`usePromptDerivations` extraction from `AppShell.tsx`, the
`useBudgetLayoutState` hook for `BudgetPage.tsx`, the shared
`touch-gestures.ts` axis-discrimination helper, the
`useHistoryEntryActions` + `useRowMutations` extractions from
`AppShell.tsx`, the `<FormSection>` extraction, and the
`useResetOnOpen` hook. The remaining touch-gesture hooks were
audited and deemed too divergent to merge further (React synthetic
vs native events, damping, async, different gates).

- **`useStorageBackend.ts` (1256 lines)**: split into `useDropboxAuth`, `useGdriveAuth`, `useFolderHandle`, leaving the main hook as an orchestrator. Natural seams at `buildInnerAdapter` lines 99–120 — each branch is ~15 lines with its own token-refresh / permission logic. Each sub-hook becomes <300 lines and testable in isolation. Storage hot path — needs its own test plan in the PR.
- **`useUserDataStorage.ts` (1104 lines)**: extract `useConflictResolution` and `useDeviceAuth` hooks to leave a leaner main hook. The current proposal is vague because the hook uses a flat action-reducer pattern with debounce, revision, and undo-stack state braided together — disentangle the reducer first (e.g. `conflictResolutionReducer`, `authErrorReducer`) before splitting into sibling hooks, otherwise the split just moves the braid.
- **`styles.css` (1604 lines)**: break into imported sub-files — `_theme-vars.css`, `_tailwind-overrides.css`, `_components.css`, `_utilities.css` — and audit for unused rules via DevTools coverage. Palette blocks (One Dark / Light / Dracula / Monokai / GitHub / Solarized / Quiet Light / System) should stay together in a single `_palettes.css` since they share a `:root[data-theme]` pattern. CSS split is a distinct skill from TS module splits; pay attention to import order at the entry point because `@layer components` rules consume color vars declared in `@theme`.
- **Modal form-init pattern (partial — minimal extraction done)**: the reset-on-open `useEffect` boilerplate has been hoisted into `useResetOnOpen(open, resetKey, reset)` in `src/hooks/`, used by `EditEntryModal` and `EditRowModal`. The full `useModalFormInit<T>` that also owns `useState` declarations was investigated and deferred — it would require renaming hundreds of JSX references from `description` to `values.description` across three large modals (700+ lines combined), which exceeds the boilerplate savings. `BudgetMetadataModal` has a per-entry-change reset (not on-open), so it doesn't fit `useResetOnOpen`; the stepping modal pattern is the right abstraction there but extraction was rejected upstream (see next bullet).
- **Step-through modal pattern — do not pursue**: a 2026-05 audit verified only `ReconciliationModal` and `BudgetMetadataModal` are true stepping modals (`HistoryModal` is read-only; `HistoryEntryEditModal` is single-entry). The two stepping shapes diverge enough — Reconciliation manages orphan / candidate decisions via `setChecked` / `setOrphanDecisions` / `setSeriesRulesById`; BudgetMetadata walks `needsMetadata()` entries with grouping / month-header logic — that the extraction's risk exceeds its line savings. Left in place by design.
- **`SettingsModal.tsx` tab registry — landed (partial)**: the `TAB_REGISTRY` array lives in `SettingsModal/tabs/index.ts` and owns every tab's id + icon + visibility gate (devMode / captureLogs). The modal walks it to derive `tabIds` and `useTabDefs`. The `Component` slot wasn't unified into the registry because each tab takes a distinct prop shape (the parent passes different deps per arm); a generic `Component` would need a "god context" prop bag that obscures rather than helps. Adding a new tab is now: append one registry entry + author the tab component.
- **`AppShell.tsx` further splits**: `useHistoryEntryActions` and `useRowMutations` landed (saving ~110 lines). The page-routing switch (`activeSheet.type === ...`) and the remaining dispatch-wiring JSX glue stay in `AppShell.tsx` — that's the file's reason for existing. Re-survey for new seams before claiming a third extraction.
- **`<FormSection>` adoption (incremental)**: the `<FormSection label="…">` component now lives at `src/components/form/FormSection.tsx` and has been adopted by `SheetModal`, `AccountModal`, `TransferModal`, `DownloadModal`, and `MatchRuleModal` (26 call sites collapsed). The other ~9 candidate modals either wrap a styled container (rounded border + `bg-surface-*`) or use a tighter `gap-1` layout, and weren't migrated. New modals should reach for `<FormSection>` from day one.
- **`<Amount value={n} settings={s} />` component — do not pursue**: a 2026-05 audit found only **3** call sites actually pair `withCurrency(formatNumber(…))` (BudgetViewerModal, TransferSearchModal, format.ts), not the ~20 originally claimed. A dedicated component doesn't earn its keep at three sites. Left in place by design.
- **`useListboxKeyboard()` hook — defer**: a 2026-05 audit found only **1** real call site (`form/SelectPicker.tsx:124–161`). `SettingsModal/admin.tsx` has icon buttons but no keyboard nav; `FormulaVariableHelper.tsx` uses plain `onClick` handlers. Premature at one call site — revisit when a second picker implements keyboard nav.
- **i18n namespace split — partial (achievements landed, settings deferred)**: `achievements.ts` (402 lines) has been split into `src/i18n/locales/{en,sv}/achievements/` with `shell.ts` (star button, unlock toast, four-tier tour) + `catalog.ts` (the per-achievement entries — ~350 lines, the bulk of the file). Adding a new achievement opens one ~350-line catalog file instead of scrolling past the chrome. The shape stays the same at the call site (`t("achievements.catalog.firstSteps.name")`) — the index spreads `shell` and nests `catalog`. The companion `settings.ts` split (387 lines) was investigated and deferred: the file is well-organized as one section per top-level key, so an 8–11 file split would add file churn without proportional discoverability gain (the navigation cost of opening one tab's section among 11 sibling files exceeds the cost of scrolling to it inside the current monolithic file).
- **`validate/` createEnumValidator polish — landed**: the `validateEnum<T>(value, allowed, fallback)` helper lives at `src/data/validate/helpers.ts` and replaced the recurring `typeof raw.x === "string" && SET.has(raw.x as T) ? raw.x : default` pattern at 9 sites (settings.ts, theme.ts, sheet.ts, account.ts). The companion `softValidate` wrapper remains unimplemented by design — only ~3 try-catch fallback sites exist in the tree today, below the threshold for a dedicated helper.
- **Cloud-adapter helpers (Dropbox 494 + GDrive 796) — do not pursue**: a 2026-05 audit confirmed the parallel 4xx semantics diverge intentionally (Dropbox 409 = path-not-found → return null; GDrive 412 = If-Match conflict → re-read remote) and the only genuinely shared surface is the bearer header. GDrive already has `authHeader()` at `gdrive-adapter.ts:148`; Dropbox repeats the one-line literal at 5 call sites. Extracting a `cloud-adapter-helpers.ts` module for one line of logic would obscure the divergence without simplifying call sites. Left in place by design.
