...
`useBudgetLayoutState` hook for `BudgetPage.tsx`, and the shared
`touch-gestures.ts` axis-discrimination helper. The remaining touch-
gesture hooks were audited and deemed too divergent to merge further
(React synthetic vs native events, damping, async, different gates).
...
- **Modal form-init pattern**: `EditEntryModal` (722), `EditRowModal` (593), `BudgetMetadataModal` (514) all repeat the "snapshot initial value on open → useEffect to pre-populate → reset-on-close" boilerplate. Extract `useModalFormInit<T>({ initialValue, fields })` that returns `{ values, setters, reset, dirty }`. New abstraction — design pass first (decide whether `dirty` is a boolean or a per-field set).
- **Step-through modal pattern**: `ReconciliationModal` (729), `HistoryModal` (466), `HistoryEntryEditModal` (259), `BudgetMetadataModal` (514) all walk a queue with skip / save / next + progress counters. Extract `useSteppingModal({ entries, filter, getProgress })`. Filter predicates are domain-specific (resolveEntryLabels for history, orphan-check for reconciliation) so the hook accepts a filter function rather than hard-coding one; grouping / month-header logic stays in each caller.
...
- **`AppShell.tsx` (still 1942 lines after the `usePromptDerivations` extraction)**: most of what remains is dispatch-wiring callbacks and the page-routing JSX, but two more hook splits would tighten things further. (a) `useHistoryEntryActions({ activeAccountId, historyEditPrompt, dispatch, setHistoryEditPrompt })` would absorb `onSubmitHistoryEdit` + `onSetHistoryEntryPrimaryIncome` (~40 lines). (b) `useRowMutations({ sheetId, itemId, dispatch })` would bundle the per-row dispatch callbacks (`onUpdateCell`, `onCommitCell`, `onSetFiscalMonthShift`, `onSetSeriesPrimaryIncome`, the merchant-hint clearers) into one return — ~100 lines. The page-routing switch itself (`activeSheet.type === ...`) stays in `AppShell.tsx` — that's the file's reason for existing.
- **`<ModalSection title="…">` extraction**: ~10+ modal call sites (`SheetModal`, `ActionHistoryModal`, `EditEntryModal`, `BulkEditModal`, …) repeat `<div className="flex flex-col gap-1.5"><span className="text-xs text-muted">{label}</span>{content}</div>`. A `<ModalSection>` wrapper drops the boilerplate so call sites read as `<ModalSection title="Name"><Input /></ModalSection>`. Pure presentational, minimal coupling risk.
- **`<Amount value={n} settings={s} />` component**: ~20–25 of the 64 `formatNumber` / `withCurrency` call sites pair them as `withCurrency(formatNumber(Math.abs(v), settings), settings)`. A small component collapses the verbose call-site math (Math.abs, nesting) and centralises the per-context variant flags (`alwaysTwoFractionDigits` for balances, `alwaysAbbreviate` for the running-balance column). Needs a small design pass on the variant prop shape.
- **`useListboxKeyboard()` hook**: `form/SelectPicker.tsx:124–161` and `SettingsModal/admin.tsx` implement nearly identical ArrowUp / ArrowDown / Home / End state machines for `role="listbox"` keyboard navigation. ~40-line hook covers both plus `FormulaVariableHelper.tsx`, which is the next picker likely to grow keyboard nav.
