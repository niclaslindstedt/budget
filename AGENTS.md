...
3. Add an entry to `SHEET_TYPE_REGISTRY` in `src/data/sheet-types/`
   so the "new sheet" picker offers the new type.
...
| New page type (savings, loans, utility, …)                     | New `src/components/<page>/` dir + new arm in `AppShell.tsx`'s routing switch + new literal in `SheetType` in `src/data/types.ts` + entry in `SHEET_TYPE_REGISTRY` in `src/data/sheet-types/`. See "Pages and the Sheet abstraction" below.                                                                                                                              |
...
| New persisted storage key                                      | Route through `nsKey` / `nsCloudPath` / `nsIdbName` in `src/data/constants/storage.ts`                                                                                                                                                                                                                                                                                   |
...
| New language                                                   | `src/i18n/locale.ts` (`Lang` union, `bcp47`, `detectInitialLanguage`), `src/i18n/locales/<code>/` directory (one file per namespace, plus `index.ts`), `src/i18n/index.ts` (`CATALOG_LOADERS` dynamic-import entry — code-split), `src/data/constants/currency.ts` (`SUPPORTED_LANGUAGES`), `src/components/LanguagePicker.tsx` (flag button). See "Translations" below. |
...
| Adding, removing, or changing a user-facing feature, or what data the app sends to a third party (Dropbox / Google Drive scopes) | The `/home` showcase page (`src/components/HomePage.tsx`) — it is the Google-facing app homepage, so its functionality description and "Why Budget asks to access your cloud storage" section must stay accurate. See "App homepage (`/home`)" below.                                                                                                                      |
...
2. Mirror the code in `src/data/constants/currency.ts`
   (`SUPPORTED_LANGUAGES`); the validator at `src/data/validate.ts`
   picks it up via the constant.
...
4. Register the new catalog in `CATALOG_LOADERS` in
   `src/i18n/index.ts` with a dynamic `import()` thunk
   (`<code>: () => import("./locales/<code>").then((m) => m.<code>)`).
   English is the only statically-bundled catalog; every other
   language is code-split and loaded on demand, so the `CATALOG_LOADERS`
   record is keyed on `Exclude<Lang, "en">` and won't compile until the
   loader is added. Skipping this is a type error, not a silent
   first-paint regression.
5. Add a flag SVG + button to `src/components/LanguagePicker.tsx`.
...
6. Add per-language month-name arrays to `MONTH_SHORT_BY_LANG` in
...
- `src/components/HomePage.tsx` body — the `/home` showcase / app
  homepage. Marketing + OAuth-compliance copy reviewed as a whole,
  English-only for the same reason as the privacy policy. See "App
  homepage (`/home`)" below.
...
The app itself is served at `/` (the SPA). Separately, `/home` is a
static **showcase / landing page** rendered by
`src/components/HomePage.tsx`. It exists to satisfy the requirements
Google imposes on an OAuth app's homepage, so it is **not** decorative
— treat it as a compliance surface that must stay truthful:

- **Accurately identify the app / brand** — name, tagline, author, and
  the verified domain it is served from.
- **Fully describe the functionality** — what every sheet type tracks.
  The feature list is rendered from `SITE_FEATURES` in
  `src/seo/siteConfig.ts`, so adding a capability there keeps the page
  current; prose paragraphs are hand-written and must be edited too.
- **Explain why the app requests user data** — the "Why Budget asks to
  access your cloud storage" section spells out the Dropbox _App
  folder_ scope and the Google Drive `drive.file` scope, that sync is
  optional, and that the developer runs no server. **This is the part
  Google verifies against the consent screen — keep it in lockstep
  with the actual OAuth scopes** in `src/storage/dropbox-adapter.ts` /
  `src/storage/gdrive-adapter.ts` and with `PrivacyPage.tsx`.
- **Link to the privacy policy** (`/privacy/`) and stay reachable
  **without signing in**.

How it is wired (mirror the privacy page, which works the same way):

- **Route + SEO** — `SHOWCASE_ROUTE` in `src/seo/routes.ts` (path
  `/home/`), added to the alias list in `emitPathAliasWithSeo(...)` in
  `vite.config.ts`. The build emits `dist/home/index.html` with its own
  `<title>` / description / canonical / JSON-LD, and a sitemap +
  `llms.txt` entry.
- **Runtime mount** — the path switch in `src/main.tsx` mounts
  `HomePage` when the pathname ends with `/home` (matches both
  production `/home` and the preview slot `/preview/home`).
- **English-only** — like `PrivacyPage`, it is hardcoded, not routed
  through the i18n catalogs (see "What's intentionally not
  translated").

**Keep it in sync.** Whenever you add, remove, or change a user-facing
feature — or change what data leaves the device (an OAuth scope, a new
third-party integration) — update `HomePage.tsx` (and the matching
`SHOWCASE_ROUTE` copy) in the same PR so the Google-facing homepage
never describes an app that no longer exists. See the row in
"Documentation sync points".

## Service-worker rollout invariants

The app installs as a PWA from every deploy slot. Production at `/`,
staging at `/preview/`, and the optional staging slot at `/branch/`
each register as **independent apps** on any device — never one
shared install. Every identity-bearing field branches per-slot inside
`pwaPlugin()` in `vite.config.ts`:

- `manifest.id`, `manifest.scope`, `manifest.start_url` — `/`,
  `/preview/`, or `/branch/`. Distinct `id` is what makes Chrome /
  Android treat the installs as different apps; without it they
  dedupe. The branch slot's identity is **stable across
  feature-branch swaps** — parking a new ref in `/branch/` arrives
  as a new SW build, not a new app, so the installed tile stays
  put.
- `manifest.name` / `short_name` — `Budget` vs `Budget (preview)` /
  `Budget pre` vs `Budget (branch)` / `Budget br`.
- `workbox.cacheId` — `budget` vs `budget-preview` vs
  `budget-branch`. Sets the Cache Storage namespace prefix so the
  slots can't collide.
- `apple-mobile-web-app-title` — patched from `Budget` to `Budget pre`
  (preview) or `Budget br` (branch) by the `patchAppleTitle` Vite
...
3. If a `branch_ref` workflow_dispatch input is set, checks out
   that ref, builds with `VITE_BASE_PATH=/branch/`, and force-pushes
   the resulting `dist/` to the auto-managed orphan branch
   `branch-deploy` in this repo.
4. Always rehydrates `dist/branch/` from the `branch-deploy` orphan
   branch (if it exists), so previously-dispatched builds persist
   across regular deploys.
5. Merges the resulting `dist/` trees into one Pages artifact,
...
- `Disallow: /branch/` lines so well-behaved crawlers skip both
...
**The `/branch/` slot is opt-in, stable, and sticky.** Its URL is
always `https://budget.niclaslindstedt.se/branch/` — install the
PWA once and let new dispatches arrive as ordinary SW updates,
exactly like production. A maintainer dispatches `pages.yml` with
`branch_ref` set to the feature branch they want to park there; the
workflow builds that ref, force-pushes the artifact to the
`branch-deploy` orphan branch in this repo, and every subsequent
`pages.yml` run (including push to `main` and the release chain)
rehydrates `/branch/` from that orphan branch. The slot persists
across regular deploys until another dispatch overwrites it. To
clear the slot, delete the `branch-deploy` branch manually — the
next Pages run will ship the artifact without `/branch/` and the
installed PWA will go offline-only on its last cached build.
...
the preview slot, `"branch"` for the `/branch/` slot) to the
bundle. The namespace string drives a single constant inside
...
- `nsCloudPath(path)` — for cloud storage paths and bare cloud
  ...
  `budget-preview-backups` folder in My Drive. The `/branch/` slot
  gets `/branch/budget.json` and `budget-branch.json` respectively.
- `nsIdbName(name)` — for IndexedDB database names. The
  FileSystem-handle DB becomes `budget-folder-handles-preview` (or
  `budget-folder-handles-branch`).

The `/branch/` namespace is **stable across feature-branch swaps**,
so the installed `/branch/` PWA's data carries forward when a new
ref is parked. The cost is that a branch shipping a breaking schema
change will be read by whatever lands next — keep an eye on
forward-only migrations or be ready to wipe the `/branch/` PWA's
storage when testing schema breaks.
...
automatically. The `/branch/` slot requests
`https://budget.niclaslindstedt.se/branch` likewise.
**Manual one-time setup:** add both URLs to the authorized
redirect URI list on the Dropbox app console and the Google Cloud
OAuth consent screen, or the slots' "Connect" buttons return an
`unauthorized redirect` error from the provider. Documented in the
release skill's pre-flight checklist.
...
| `find-optimizations` | User asks for high-value performance wins — order-of-magnitude improvements, not micro-tuning. Surveys per-keystroke / per-render / per-load hot paths, presents candidates with complexity and trigger context before any work, and is honest when the codebase has already been tuned and no high-value wins remain. Pure perf changes ship `no-changelog`.                                                          | manual    |
| `fix-comments`       | User wants comments cleaned up, or you noticed the file you're editing narrates history ("previously", "the legacy behaviour", "see the plan") instead of explaining current invariants. Removes or rewrites changelog-style comments while keeping WHY-comments and anti-pattern guards ("don't try X — it doesn't work"). Scoped to one module per PR.                                                               | manual    |
...
`debug-from-logs`, `tune-pwa-icons`, `design`, `refactor`,
`find-optimizations`, and `fix-comments` skills are manual
...
from feature work; perf and comment hygiene are explicit user
asks, not background sync), so none of them are part of the
