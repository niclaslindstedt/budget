...
in the ledger instead of collapsing to a bare "(no label)". The "Probable
matches" section header carries a "Check all" / "Uncheck all" toggle
(shown only when there is more than one candidate) so a long match list
doesn't have to be confirmed checkbox by checkbox; the label flips to
"Uncheck all" once every candidate is selected.
