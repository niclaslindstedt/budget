...
| **Visualize spending** / **spending dashboard**                                                 | `BudgetSpendingModal.tsx`; `collectSpendingFacts` + friends (`src/data/budget/spending.ts`); `DonutChart` / `StackedBarChart` / `LineChart` (`src/components/charts/`). Opened from the budget sheet's title "…" menu. [→](overview.md#visualize-spending) |
| **Spread item costs** / **lifetime spreading** / **avskrivning**                                | The cogwheel chart option in `BudgetSpendingModal.tsx`; `SpendingInputs.spreadItemCosts` (`src/data/budget/spending.ts`), `Item.lifetimeYears`. [→](overview.md#visualize-spending)                                                                        |
...
`src/data/property-transfer/` (sale-handover export / import), with the
shared mortgage math (amortisation, interest, payment split) in
`src/data/finance/`.
...
| **Sold property** / **sale date** / **previously owned** | `Property.soldDate` / `Property.soldAmount` (`src/data/types/properties.ts`); `isPropertySoldAt` (`src/data/property-value/value.ts`); `PropertyEditorModal.tsx`, `PropertyCard.tsx`. [→](overview.md#sold-property)                                                 |
...
| **Mortgage rate change**                                 | `MortgageRateChange` (`Mortgage.rateHistory`); `resolveRateAt` (`src/data/finance/interest.ts`). [→](overview.md#mortgage-rate-change)                                                                                                                               |
| **Mortgage payment**                                     | `MortgagePayment` (`Mortgage.payments`); `splitPaymentAcrossMortgages`, `splitRecordedPayment` (`src/data/finance/payment.ts`). [→](overview.md#mortgage-payment)                                                                                                    |
...
| **Time-range buttons** / **range row** (charts)          | `ChartRangeRow` / `chartRangeCutoffMs` / `DEFAULT_CHART_RANGE` (`src/components/charts/ChartRangeRow.tsx`); used by `LoansChartModal.tsx` and `InsightsPage.tsx`. [→](overview.md#time-range-buttons)                                                                |
...
| **Remaining balance** / **monthly payment** (loan) | `loanRemainingBalance` / `loanPaidSoFar` / `loanMonthlyPayment` (`src/data/loans/balance.ts`); anchors on `Loan.balanceHistory` + `Loan.startSum`. [→](overview.md#loan-remaining-balance)                                                               |
| **Update balance** (loan) / **balance snapshot**   | `LoanUpdateBalanceModal.tsx`; `Loan.balanceHistory`, `LoanBalancePoint` (`src/data/types/loans.ts`); `addLoanBalance` / `deleteLoanBalance` (`src/data/reducers/loans.ts`). Opened from the loan row's "…" menu. [→](overview.md#update-balance-loan)    |
...
| **Lifetime** (item) / **useful life**                         | `Item.lifetimeYears` (`src/data/types/items.ts`); edited in `ItemEditorModal`, consumed by `collectSpendingFacts` (`src/data/budget/spending.ts`). [→](overview.md#item)                           |
...
| **Action column compaction** / **collapsed actions column** / **compact ⋯ menu** | `src/hooks/useActionsCompaction.ts`, `ActionsCompactContext.ts`. [→](overview.md#action-column-compaction)                                                                                                                                                     |
